...
$base = Get-AacBuildVersion $ReleaseDate
...
if ($ReleaseRequested) {
    ...
    $versions = @($tags | ForEach-Object { Read-AacVersionTag $_ })
    $occupied = @($versions) + @($releaseTags | ForEach-Object { Read-AacVersionTag $_ })
...
Write-Output "Selected AAC build version $version"
