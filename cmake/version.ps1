...
function Read-AacVersionTag([string]$Name) {
...
function Get-AacBuildVersion([string]$Version, [string]$SourceDirectory = (Split-Path $PSScriptRoot -Parent)) {
    if (-not $Version) {
    ...
    $parsed = Read-AacVersionTag $Version
...
function Assert-AacFileVersion([string]$Path, [string]$Version) {
    $parsed = Get-AacBuildVersion $Version
